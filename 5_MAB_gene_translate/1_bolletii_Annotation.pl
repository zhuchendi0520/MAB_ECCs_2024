...
open F1, "</home/zcd/script/NTM_mapping/MAB.subsp.bolletii/translate/2_MAB.subsp.bolletii_20220725" or die $!; #open Tuberculist_new_20150307
while(<F1>){
	chomp;
	@a=split "\t",$_;
	$start{$a[0]}=$a[3];
...
close F1;
$last_igr="I3U44_RS26685-I3U44_RS00005";
$igrha{$last_igr}=1;
push(@igr,$last_igr);
$start{$last_igr}=5244660;
$end{$last_igr}=5245037;
...
open F3, "</home/zcd/script/NTM_mapping/MAB.subsp.bolletii.fna" or die $!; #open genome.fasta
while(<F3>){
	chomp;
	if($_=~m/>/){
		}
...
$type="";				
		...
		foreach $j(@igr){
			if(($a[0]>=$start{$j})&&($a[0]<=$end{$j})){
				@b=split "-",$j;
				if($j eq "I3U44_RS26685-I3U44_RS00005"){
				$left=$a[0]-$start{$j};
				$right=5066832-$a[0];
				}else{
				$left=$a[0]-$end{$b[0]};
