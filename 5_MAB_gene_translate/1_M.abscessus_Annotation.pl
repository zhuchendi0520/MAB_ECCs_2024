...
open F1, "</home/zcd/script/NTM_mapping/M.abscessus/translate/2_M.abscessus_20220725" or die $!; #open Tuberculist_new_20150307
while(<F1>){
	chomp;
	@a=split "\t",$_;
	$start{$a[0]}=$a[3];
...
open F2, "</home/zcd/script/NTM_mapping/M.abscessus/translate/3_genetic_codes" or die $!; #open genetic_code
while(<F2>){
	chomp;
	@a=split "\t",$_;
	$code{$a[0]}=$a[1];
...
open F3, "</home/zcd/script/NTM_mapping/M.abscessus.fna" or die $!; #open genome.fasta
while(<F3>){
	chomp;
	if($_=~m/>/){
		}
...
$type="";				
		...
		foreach $j(@igr){
			if(($a[0]>=$start{$j})&&($a[0]<=$end{$j})){
				@b=split "-",$j;
				if($j eq "EFV83_RS25425-EFV83_RS00085"){
				$left=$a[0]-$start{$j};
				$right=5066832-$a[0];
				}else{
				$left=$a[0]-$end{$b[0]};
