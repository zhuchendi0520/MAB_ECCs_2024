...
open F1, "</home/zcd/script/NTM_mapping/MAB.subsp.massiliense/translate/2_MAB.subsp.massiliense_20221024" or die $!; #open Tuberculist_new_20150307
while(<F1>){
	chomp;
	@a=split "\t",$_;
	$start{$a[0]}=$a[3];
...
close F1;
$last_igr="COLCPH_24910-COLCPH_00005";
$igrha{$last_igr}=1;
push(@igr,$last_igr);
$start{$last_igr}=5065897;
$end{$last_igr}=5066832;

@all=(@gene,@igr);

open F2, "</home/zcd/script/NTM_mapping/MAB.subsp.massiliense/translate/3_genetic_codes" or die $!; #open genetic_code
while(<F2>){
	chomp;
	@a=split "\t",$_;
	$code{$a[0]}=$a[1];
...
open F3, "</home/zcd/script/NTM_mapping/MAB.subsp.massiliense/MAB.subsp.massiliense.fna" or die $!; #open genome.fasta
while(<F3>){
	chomp;
	if($_=~m/>/){
		}
...
$type="";				
		...
		foreach $j(@igr){
			if(($a[0]>=$start{$j})&&($a[0]<=$end{$j})){
				@b=split "-",$j;
				if($j eq "COLCPH_24910-COLCPH_00005"){
				$left=$a[0]-$start{$j};
				$right=5066832-$a[0];
				}else{
				$left=$a[0]-$end{$b[0]};
